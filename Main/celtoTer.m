
%celestial dan terrainde dönüşüm yaptık
clc; clear all;

% GIRDILER
YYYY = 2023;
MM = 06;
DD = 06;             
HH = 11;
Mi = 48; 


%UT1-UTC = 0.5225833;

SS = 24 + 0.5225833;  %UT1 %saniye

xp = 0.03;        % yay saniyesi dereceye fonksiyonda ceviriliyor
yp = 0.4;        % yay saniyesi dereceye fonksiyonda ceviriliyor

E = referenceEllipsoid('grs80');

lat = dms2degrees([39 17 42.36]);
lon = dms2degrees([32 44 21.57]);
h = 758.653;

% Ciktilar
%% a
P = precession(YYYY, MM, DD, HH, Mi, SS)


N = nutation(YYYY, MM, DD, HH, Mi, SS)
%% b

Rm = pol_motion(xp, yp) % kutup hareketi

%% c
Teta = GAST(YYYY, MM, DD, HH, Mi, SS)
degrees2dms(Teta)

%% d 

Rs=R3(Teta)

%% e
Last = LAST(YYYY,MM,DD,HH, Mi, SS, lat, lon)
degrees2dms(Last)

%[X,Y,Z] = geodetic2ecef(E,lat,lon,h);
XYZ_cel=[4121952.0008,2738143.5753,4012082.2287];

%% f
XYZ_ter=XYZ_cel*(Rm*Rs*N*P)